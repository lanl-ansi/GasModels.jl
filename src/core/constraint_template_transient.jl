...
function constraint_pipe_momentum_balance(gm::AbstractGasModel, i::Int, nw::Int = gm.cnw)
    ...
    fr_junction = pipe["fr_junction"]
    to_junction = pipe["to_junction"]
    resistance = _calc_pipe_resistance_rho_phi_space(pipe, gm.ref[:it][_gm_it_sym][:base_length])
...
function constraint_storage_well_momentum_balance(
    ...
    beta = (-2.0 * gm.ref[:it][_gm_it_sym][:base_length] * 9.8 * length_per_well_segment) / gm.ref[:it][_gm_it_sym][:sound_speed]^2
    resistance = well["well_friction_factor"] * gm.ref[:it][_gm_it_sym][:base_length] * length_per_well_segment / well["well_diameter"]
