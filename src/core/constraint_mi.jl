...
function constraint_mass_flow_balance(gm::AbstractMIModels, n::Int, i, f_pipes, t_pipes, f_compressors, t_compressors, f_resistors, t_resistors, f_short_pipes, t_short_pipes, f_valves, t_valves, f_control_valves, t_control_valves, fl_constant, fg_constant, consumers, producers, flmin, flmax, fgmin, fgmax)
    ...
    add_constraint(gm, n, :junction_mass_flow_balance, i, JuMP.@constraint(gm.model, fg_constant - fl_constant + sum(fg[a] for a in producers) - sum(fl[a] for a in consumers) ==
...
function constraint_mass_flow_balance_ne(gm::AbstractMIModels, n::Int, i, f_pipes, t_pipes, f_compressors, t_compressors, f_resistors, t_resistors, f_short_pipes, t_short_pipes, f_valves, t_valves, f_control_valves, t_control_valves, f_ne_pipes, t_ne_pipes, f_ne_compressors, t_ne_compressors, fl_constant, fg_constant, consumers, producers, flmin, flmax, fgmin, fgmax)
    ...
    add_constraint(gm, n, :junction_mass_flow_balance_ne_ls, i, JuMP.@constraint(gm.model, fg_constant - fl_constant + sum(fg[a] for a in producers) - sum(fl[a] for a in consumers) ==
                                                                                      sum(f_pipe[a] for a in f_pipes) - sum(f_pipe[a] for a in t_pipes) +
                                                                                      sum(f_compressor[a] for a in f_compressors) - sum(f_compressor[a] for a in t_compressors) +
                                                                                      sum(f_resistor[a] for a in f_resistors) - sum(f_resistor[a] for a in t_resistors) +
                                                                                      sum(f_short_pipe[a] for a in f_short_pipes) - sum(f_short_pipe[a] for a in t_short_pipes) +
...
function constraint_pipe_pressure(gm::AbstractMIModels, n::Int, k, i, j, pd_min, pd_max)
    ...
    pi = var(gm,n,:p,i)
    pj = var(gm,n,:p,j)
    add_constraint(gm, n, :on_off_pressure_drop1, k, JuMP.@constraint(gm.model, (1-y) * pd_min <= pi - pj))
    add_constraint(gm, n, :on_off_pressure_drop2, k, JuMP.@constraint(gm.model, pi - pj <= y * pd_max))
end
...
function constraint_pipe_mass_flow(gm::AbstractMIModels, n::Int, k, f_min, f_max)
    y = var(gm,n,:y_pipe,k)
    f = var(gm,n,:f_pipe,k)
    add_constraint(gm, n, :on_off_pipe_flow1, k, JuMP.@constraint(gm.model, (1-y) * f_min <= f))
    add_constraint(gm, n, :on_off_pipe_flow2, k, JuMP.@constraint(gm.model, f <= y * f_max))
...
function constraint_pipe_pressure_ne(gm::AbstractMIModels, n::Int, k, i, j, pd_min, pd_max)
    ...
    pi = var(gm,n,:p,i)
    pj = var(gm,n,:p,j)
    add_constraint(gm, n, :on_off_pressure_drop_ne1, k, JuMP.@constraint(gm.model, (1-y) * pd_min <= pi - pj))
    add_constraint(gm, n, :on_off_pressure_drop_ne2, k, JuMP.@constraint(gm.model, pi - pj <= y * pd_max))
end
...
function constraint_pipe_mass_flow_ne(gm::AbstractMIModels, n::Int, k, f_min, f_max)
    ...
    add_constraint(gm, n, :on_off_pipe_flow_ne1, k, JuMP.@constraint(gm.model, (1-y)*f_min <= f))
    add_constraint(gm, n, :on_off_pipe_flow_ne2, k, JuMP.@constraint(gm.model, f <= y*f_max))
...
function constraint_short_pipe_mass_flow(gm::AbstractMIModels, n::Int, k, f_min, f_max)
    ...
    add_constraint(gm, n, :on_off_short_pipe_flow1, k, JuMP.@constraint(gm.model, f_min*(1-y) <= f))
    add_constraint(gm, n, :on_off_short_pipe_flow2, k, JuMP.@constraint(gm.model, f <= f_max*y))
...
function constraint_compressor_mass_flow(gm::AbstractMIModels, n::Int, k, f_min, f_max)
    ...
    add_constraint(gm, n, :on_off_compressor_flow_direction1, k, JuMP.@constraint(gm.model, (1-y)*f_min <= f))
    add_constraint(gm, n, :on_off_compressor_flow_direction2, k, JuMP.@constraint(gm.model, f <= y*f_max))
...
function constraint_compressor_ratios(gm::AbstractMIModels, n::Int, k, i, j, min_ratio, max_ratio, i_pmax, j_pmax)
    ...
    pi = var(gm,n,:p,i)
    pj = var(gm,n,:p,j)
    add_constraint(gm, n, :on_off_compressor_ratios1, k, JuMP.@constraint(gm.model, pj - max_ratio^2*pi <= (1-y)*(j_pmax^2)))
    add_constraint(gm, n, :on_off_compressor_ratios2, k, JuMP.@constraint(gm.model, min_ratio^2*pi - pj <= (1-y)*(i_pmax^2)))
    add_constraint(gm, n, :on_off_compressor_ratios3, k, JuMP.@constraint(gm.model, pi - pj <= y*(i_pmax^2)))
    add_constraint(gm, n, :on_off_compressor_ratios4, k, JuMP.@constraint(gm.model, pj - pi <= y*(j_pmax^2)))
end
...
function constraint_compressor_mass_flow_ne(gm::AbstractMIModels, n::Int, k, f_min, f_max)
    ...
    add_constraint(gm, n, :on_off_compressor_flow_direction_ne1, k, JuMP.@constraint(gm.model, (1-y)*f_min <= f))
    add_constraint(gm, n, :on_off_compressor_flow_direction_ne2, k, JuMP.@constraint(gm.model, f <= y*f_max))
...
function constraint_compressor_ratios_ne(gm::AbstractMIModels, n::Int, k, i, j, min_ratio, max_ratio, f_max, i_pmin, i_pmax, j_pmin, j_pmax)
    ...
    add_constraint(gm, n, :on_off_compressor_ratios_ne1, k, JuMP.@constraint(gm.model,  pj - (max_ratio^2*pi) <= (2-y-zc)*j_pmax^2))
    add_constraint(gm, n, :on_off_compressor_ratios_ne2, k, JuMP.@constraint(gm.model,  (min_ratio^2*pi) - pj <= (2-y-zc)*(min_ratio^2*i_pmax^2)))
    add_constraint(gm, n, :on_off_compressor_ratios_ne3, k, JuMP.@constraint(gm.model,  pi - (max_ratio^2*pj) <= (1+y-zc)*i_pmax^2))
    add_constraint(gm, n, :on_off_compressor_ratios_ne4, k, JuMP.@constraint(gm.model,  (min_ratio^2*pj) - pi <= (1+y-zc)*(min_ratio^2*j_pmax^2)))
end
...
function constraint_on_off_valve_mass_flow(gm::AbstractMIModels, n::Int, k, f_min, f_max)
    ...
    add_constraint(gm, n,:on_off_valve_flow_direction1, k, JuMP.@constraint(gm.model, f_min*(1-y) <= f))
    add_constraint(gm, n,:on_off_valve_flow_direction2, k, JuMP.@constraint(gm.model, f <= f_max*y))
    add_constraint(gm, n,:on_off_valve_flow_direction3, k, JuMP.@constraint(gm.model, f_min*v <= f))
    add_constraint(gm, n,:on_off_valve_flow_direction4, k, JuMP.@constraint(gm.model, f <= f_max*v))
...
function constraint_on_off_control_valve_mass_flow(gm::AbstractMIModels, n::Int, k, f_min, f_max)
    ...
    add_constraint(gm, n, :on_off_control_valve_flow_direction1, k, JuMP.@constraint(gm.model, f_min*(1-y) <= f))
    add_constraint(gm, n, :on_off_control_valve_flow_direction2, k, JuMP.@constraint(gm.model, f <= f_max*y))
    add_constraint(gm, n, :on_off_control_valve_flow_direction3, k, JuMP.@constraint(gm.model, f_min*v <= f ))
    add_constraint(gm, n, :on_off_control_valve_flow_direction4, k, JuMP.@constraint(gm.model, f <= f_max*v))
...
function constraint_on_off_control_valve_pressure(gm::AbstractMIModels, n::Int, k, i, j, min_ratio, max_ratio, f_max, i_pmin, i_pmax, j_pmin, j_pmax)
    ...
    pj = var(gm,n,:p,j)
    v  = var(gm,n,:v_control_valve,k)
    add_constraint(gm, n, :on_off_control_valve_pressure_drop1, k, JuMP.@constraint(gm.model,  pj - (max_ratio^2*pi) <= (2-y-v)*j_pmax^2))
    add_constraint(gm, n, :on_off_control_valve_pressure_drop2, k, JuMP.@constraint(gm.model,  (min_ratio^2*pi) - pj <= (2-y-v)*i_pmax^2))
    add_constraint(gm, n, :on_off_control_valve_pressure_drop3, k, JuMP.@constraint(gm.model,  pj - pi <= (1 + y - v)*j_pmax^2))
    add_constraint(gm, n, :on_off_control_valve_pressure_drop4, k, JuMP.@constraint(gm.model,  pi - pj <= (1 + y - v)*i_pmax^2))
end
...
function constraint_source_flow(gm::AbstractMIModels, n::Int, i, f_pipes, t_pipes, f_compressors, t_compressors, f_resistors, t_resistors, f_short_pipes, t_short_pipes, f_valves, t_valves, f_control_valves, t_control_valves)
    ...
    add_constraint(gm, n, :source_flow, i,  JuMP.@constraint(gm.model, sum(y_pipe[a] for a in f_pipes) + sum((1-y_pipe[a]) for a in t_pipes) +
...
function constraint_source_flow_ne(gm::AbstractMIModels, n::Int, i, f_pipes, t_pipes, f_compressors, t_compressors, f_resistors, t_resistors, f_short_pipes, t_short_pipes, f_valves, t_valves, f_control_valves, t_control_valves, f_ne_pipes, t_ne_pipes, f_ne_compressors, t_ne_compressors)
    ...
    y_ne_pipe       = var(gm,n,:y_ne_pipe)
    y_ne_compressor = var(gm,n,:y_ne_compressor)
    add_constraint(gm, n, :source_flow_ne, i, JuMP.@constraint(gm.model, sum(y_pipe[a] for a in f_pipes) + sum((1-y_pipe[a]) for a in t_pipes) +
                                                                    sum(y_compressor[a] for a in f_compressors) + sum((1-y_compressor[a]) for a in t_compressors) +
                                                                    sum(y_resistor[a] for a in f_resistors) + sum((1-y_resistor[a]) for a in t_resistors) +
                                                                    sum(y_short_pipe[a] for a in f_short_pipes) + sum((1-y_short_pipe[a]) for a in t_short_pipes) +
                                                                    sum(y_valve[a] for a in f_valves) + sum((1-y_valve[a]) for a in t_valves) +
...
function constraint_sink_flow(gm::AbstractMIModels, n::Int, i, f_pipes, t_pipes, f_compressors, t_compressors, f_resistors, t_resistors, f_short_pipes, t_short_pipes, f_valves, t_valves, f_control_valves, t_control_valves)
    ...
    add_constraint(gm, n, :sink_flow, i, JuMP.@constraint(gm.model, sum((1-y_pipe[a]) for a in f_pipes) + sum(y_pipe[a] for a in t_pipes) +
...
function constraint_sink_flow_ne(gm::AbstractMIModels, n::Int, i, f_pipes, t_pipes, f_compressors, t_compressors, f_resistors, t_resistors, f_short_pipes, t_short_pipes, f_valves, t_valves, f_control_valves, t_control_valves, f_ne_pipes, t_ne_pipes, f_ne_compressors, t_ne_compressors)
    ...
    add_constraint(gm, n, :sink_flow_ne, i, JuMP.@constraint(gm.model, sum((1-y_pipe[a]) for a in f_pipes) + sum(y_pipe[a] for a in t_pipes) +
                                                                  sum((1-y_compressor[a]) for a in f_compressors) + sum(y_compressor[a] for a in t_compressors) +
                                                                  sum((1-y_resistor[a]) for a in f_resistors) + sum(y_resistor[a] for a in t_resistors) +
                                                                  sum((1-y_short_pipe[a]) for a in f_short_pipes) + sum(y_short_pipe[a] for a in t_short_pipes) +
                                                                  sum((1-y_valve[a]) for a in f_valves) + sum(y_valve[a] for a in t_valves) +
...
function constraint_conserve_flow(gm::AbstractMIModels, n::Int, idx, f_pipes, t_pipes, f_compressors, t_compressors, f_resistors, t_resistors, f_short_pipes, t_short_pipes, f_valves, t_valves, f_control_valves, t_control_valves)
    ...
    for (y1, t1) in y_fr
        for (y2, t2) in y_fr
            if t1 != t2
                add_constraint(gm, n, :conserve_flow, idx, JuMP.@constraint(gm.model, y1 + y2 == 1))
            end
        end
    end
    ...
    for (y1, t1) in y_fr
        for (y2, t2) in y_to
            if t1 != t2
                add_constraint(gm, n, :conserve_flow, idx, JuMP.@constraint(gm.model, y1 == y2))
            end
        end
    end
end
...
function constraint_ne_pipe_parallel_flow(gm::AbstractMIModels, n::Int, k, num_connections, aligned_pipes, opposite_pipes, aligned_compressors, opposite_compressors,
    ...
    add_constraint(gm, n, :parallel_flow_ne, k, JuMP.@constraint(gm.model, sum(y_pipe[i] for i in aligned_pipes) + sum((1-y_pipe[i]) for i in opposite_pipes) +
