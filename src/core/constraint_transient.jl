...
function constraint_compressor_physics(gm::AbstractGasModel, nw::Int, compressor_id::Int, fr_junction::Int, to_junction::Int, type::Int, min_ratio::Float64, max_ratio::Float64)
    ...
    if type == 0
        if (min_ratio <= 1.0 && max_ratio >= 1)
            ...
            _add_constraint!(gm, nw, :compressor_physics_ratios_1, k, JuMP.@constraint(gm.model, pk - max_ratio^2 * p_fr^2 <= 0))
            _add_constraint!(gm, nw, :compressor_physics_ratios_2, k, JuMP.@constraint(gm.model, min_ratio^2 * p_fr^2 - pk <= 0))
            _add_constraint!(gm, nw, :compressor_physics_ratios_3, k, JuMP.@constraint(gm.model, f * (p_fr - pk) <= 0))
            _add_constraint!(gm, nw, :compressor_physics_ratios_4, k, JuMP.@constraint(gm.model, pk - max_ratio^2 * p_to^2 <= 0))
            _add_constraint!(gm, nw, :compressor_physics_ratios_5, k, JuMP.@constraint(gm.model, min_ratio^2 * p_to^2 - pk <= 0))
            _add_constraint!(gm, nw, :compressor_physics_ratios_6, k, JuMP.@constraint(gm.model, -f * (p_to - pk) <= 0))
            _add_constraint!(gm, nw, :compressor_physics_ratios_7, k, JuMP.@constraint(gm.model,  p_fr * alpha_1 == p_k))
            _add_constraint!(gm, nw, :compressor_physics_ratios_8, k, JuMP.@constraint(gm.model,  p_to * alpha_2 == p_k))
            _add_constraint!(gm, nw, :compressor_physics_ratios_9, k, JuMP.@constraint(gm.model,  alpha == alpha_1 + alpha_2 - 1))
