...
function objective_min_economic_costs(gm::AbstractGasModel, nws = [nw_id_default])
    ...
    compressor_energy_expression   = JuMP.@expression(gm.model, sum(sum(abs(f[n][i]) * (r[n][i]^m - 1) * work * base_flow for (i, compressor) in ref(gm, n, :compressor))
                                                                for n in nws
                                                                )
                                                     )
    
    if (economic_weighting == 1.0)
        return JuMP.@objective(gm.model, Min, supply_demand_costs_expression)
    elseif (economic_weighting == 0.0)
        z = JuMP.@variable(gm.model)
        JuMP.@constraint(gm.model, z >= compressor_energy_expression)
        return JuMP.@objective(gm.model, Min, z)                                                                    
    else
        z = JuMP.@variable(gm.model)
        JuMP.@constraint(gm.model, z >= economic_weighting * supply_demand_costs_expression + (1.0 - economic_weighting) * compressor_energy_expression)
        return JuMP.@objective(gm.model, Min, z)                                                                    
    end    
end
