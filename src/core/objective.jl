...
function objective_max_load(gm::GenericGasModel, nws=[gm.cnw])
    load_set = Dict(n => 
        keys(Dict(x for x in gm.ref[:nw][n][:consumer] if x.second["qlmax"] != 0 || x.second["qlmin"] != 0))
    for n in nws)
    fl =  Dict(n => gm.var[:nw][n][:fl] for n in nws)  
    obj = JuMP.@objective(gm.model, Max, sum(sum(gm.ref[:nw][n][:consumer][i]["priority"] *  fl[n][i] for i in load_set[n]) for n in nws))      
 end
