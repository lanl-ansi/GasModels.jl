...
function objective_min_ne_cost{T}(gm::GenericGasModel{T}, nws=[gm.cnw]; normalization=1000000.0)
    zp = Dict(n => gm.var[:nw][n][:zp] for n in nws) # gm.var[:zp] 
    zc = Dict(n => gm.var[:nw][n][:zc] for n in nws) # gm.var[:zc] 
...
#    filter(i -> gm.ref[:junction][i]["qlmin"] != gm.ref[:junction][i]["qlmax"], keys(gm.ref[:junction]))            
