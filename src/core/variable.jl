...
function variable_potential(gm::AbstractGasModel, nw::Int=nw_id_default; bounded::Bool=true, report::Bool=true)
    ...
    invert_potential = val -> (val >= 0) ? invert_positive_potential(ss, val) : invert_negative_potential(ss, val)
    ...
    if bounded
        for (i, junction) in ref(gm, nw, :junction)
            JuMP.set_lower_bound(psqr[i], get_potential(ref(gm, nw, :junction, i)["p_min"]))
            JuMP.set_upper_bound(psqr[i], get_potential(ref(gm, nw, :junction, i)["p_max"]))
        end
    end
...
function variable_flow(gm::AbstractGasModel, nw::Int=nw_id_default; bounded::Bool=true, report::Bool=true)
    ...
    variable_resistor_flow(gm, nw; bounded=bounded, report=report)
    variable_loss_resistor_flow(gm, nw; bounded=bounded, report=report)
    variable_short_pipe_flow(gm, nw; bounded=bounded, report=report)
    variable_valve_flow(gm, nw; bounded=bounded, report=report)
    variable_regulator_flow(gm, nw; bounded=bounded, report=report)
end
...
function variable_resistor_flow(gm::AbstractGasModel, nw::Int=nw_id_default; bounded::Bool=true, report::Bool=true)
...
function variable_loss_resistor_flow(gm::AbstractGasModel, nw::Int=nw_id_default; bounded::Bool=true, report::Bool=true)
...
function variable_short_pipe_flow(gm::AbstractGasModel, nw::Int=nw_id_default; bounded::Bool=true, report::Bool=true)
...
function variable_valve_flow(gm::AbstractGasModel, nw::Int=nw_id_default; bounded::Bool=true, report::Bool=true)
...
function variable_regulator_flow(gm::AbstractGasModel, nw::Int=nw_id_default; bounded::Bool=true, report::Bool=true)
