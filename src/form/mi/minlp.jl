...
#    add_constraint(gm, n, :weymouth1, k, @constraint(gm.model, w*(pi - pj) >= f^2 - (1-yp)*f_min^2)) # when yp = 0, flow goes from j -> i, therefore the biggest flow f_min^2 and constraint is deactivated
#    add_constraint(gm, n, :weymouth2, k, @constraint(gm.model, w*(pi - pj) <= f^2 + (1-yp)*f_min^2))
#    add_constraint(gm, n, :weymouth3, k, @constraint(gm.model, w*(pj - pi) >= f^2 - (1-yn)*f_max^2)) # when yn = 0, flow goes from i -> j, therefore the biggest flow f_max^2 and constraint is deactivated
#    add_constraint(gm, n, :weymouth4, k, @constraint(gm.model, w*(pj - pi) <= f^2 + (1-yn)*f_max^2))
    ...
    add_constraint(gm, n, :weymouth1, k, @constraint(gm.model, w*(pi - pj) >= f^2 - (1-yp)*2*f_min^2))
    add_constraint(gm, n, :weymouth2, k, @constraint(gm.model, w*(pi - pj) <= f^2))
    add_constraint(gm, n, :weymouth3, k, @constraint(gm.model, w*(pj - pi) >= f^2 - (1-yn)*2*f_max^2))
...
#    add_constraint(gm, n, :weymouth_ne1, k, @constraint(gm.model, w*(pi - pj) >= f^2 - (2-yp-zp)*mf^2))
#    add_constraint(gm, n, :weymouth_ne2, k, @constraint(gm.model, w*(pi - pj) <= f^2 + (2-yp-zp)*mf^2))
#    add_constraint(gm, n, :weymouth_ne3, k, @constraint(gm.model, w*(pj - pi) >= f^2 - (2-yn-zp)*mf^2))
#    add_constraint(gm, n, :weymouth_ne4, k, @constraint(gm.model, w*(pj - pi) <= f^2 + (2-yn-zp)*mf^2))

    add_constraint(gm, n, :weymouth_ne1, k, @constraint(gm.model, w*(pi - pj) >= f^2 - (2-yp-zp)*f_min^2))
    add_constraint(gm, n, :weymouth_ne2, k, @constraint(gm.model, w*(pi - pj) <= f^2 + (2-yp-zp)*f_max^2))
    add_constraint(gm, n, :weymouth_ne3, k, @constraint(gm.model, w*(pj - pi) >= f^2 - (2-yn-zp)*f_min^2))
    add_constraint(gm, n, :weymouth_ne4, k, @constraint(gm.model, w*(pj - pi) <= f^2 + (2-yn-zp)*f_max^2))
    ...
    add_constraint(gm, n, :weymouth_ne1, k, @constraint(gm.model, w*(pi - pj) >= f^2 + (1-zp)*w*pd_min - (1-yp)*2*f_min^2))
    add_constraint(gm, n, :weymouth_ne2, k, @constraint(gm.model, w*(pi - pj) <= f^2 + (1-zp)*w*pd_max) )
    add_constraint(gm, n, :weymouth_ne3, k, @constraint(gm.model, w*(pj - pi) >= f^2 - (1-zp)*w*pd_max - (1-yn)*2*f_max^2))
...
function constraint_weymouth_ne_directed(gm::GenericGasModel{T},  n::Int, k, i, j, w, pd_min, pd_max, direction) where T <: AbstractMINLPForm
    ...
    #else
    #    add_constraint(gm, n, :weymouth_ne3, k, @constraint(gm.model, w*(pj - pi) >= f^2 - (1-zp)*mf^2))
    #    add_constraint(gm, n, :weymouth_ne4, k, @constraint(gm.model, w*(pj - pi) <= f^2 + (1-zp)*mf^2))
    #end
