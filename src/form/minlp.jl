...
function variable_flow_directed(gm::AbstractMINLPModel, n::Int=gm.cnw; bounded::Bool=true, report::Bool=true, compressor=ref(gm, n, :default_compressor), short_pipe=ref(gm, n, :undirected_short_pipe), valve=ref(gm, n, :valve), regulator=ref(gm, n, :undirected_regulator))
    variable_mass_flow(gm, n; bounded=bounded, report=report)
    variable_connection_direction(gm, n; compressor=compressor, short_pipe=short_pipe, valve=valve, regulator=regulator, report=report)
end
