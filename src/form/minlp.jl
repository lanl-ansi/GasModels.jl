...
function constraint_pipe_weymouth(gm::AbstractMINLPModel, n::Int, k, i, j, f_min, f_max, w, pd_min, pd_max)
    ...
    add_constraint(gm, n, :weymouth1, k, JuMP.@constraint(gm.model, w*(pi - pj) >= f^2 - (1-y)*2*f_min^2))
    add_constraint(gm, n, :weymouth2, k, JuMP.@constraint(gm.model, w*(pi - pj) <= f^2))
    add_constraint(gm, n, :weymouth3, k, JuMP.@constraint(gm.model, w*(pj - pi) >= f^2 - y*2*f_max^2))
    add_constraint(gm, n, :weymouth4, k, JuMP.@constraint(gm.model, w*(pj - pi) <= f^2))
end
...
function constraint_pipe_weymouth_directed(gm::AbstractMINLPModel, n::Int, k, i, j, w, f_min, f_max, direction)
    ...
    if direction == 1
        add_constraint(gm, n, :weymouth1, k, JuMP.@constraint(gm.model, w*(pi - pj) >= f^2))
        add_constraint(gm, n, :weymouth2, k, JuMP.@constraint(gm.model, w*(pi - pj) <= f^2))
    else
        add_constraint(gm, n, :weymouth3, k, JuMP.@constraint(gm.model, w*(pj - pi) >= f^2))
        add_constraint(gm, n, :weymouth4, k, JuMP.@constraint(gm.model, w*(pj - pi) <= f^2))
    end
end
...
function constraint_pipe_weymouth_ne(gm::AbstractMINLPModel,  n::Int, k, i, j, w, f_min, f_max, pd_min, pd_max)
    ...
    add_constraint(gm, n, :weymouth_ne1, k, JuMP.@constraint(gm.model, w*(pi - pj) >= f^2 + (1-zp)*w*pd_min - (1-y)*2*f_min^2))
    add_constraint(gm, n, :weymouth_ne2, k, JuMP.@constraint(gm.model, w*(pi - pj) <= f^2 + (1-zp)*w*pd_max) )
    add_constraint(gm, n, :weymouth_ne3, k, JuMP.@constraint(gm.model, w*(pj - pi) >= f^2 - (1-zp)*w*pd_max - y*2*f_max^2))
...
function constraint_compressor_ratio_value(gm::AbstractMINLPModel, n::Int, k, i, j)
    ...
    pj    = var(gm,n,:p,j)
    r = var(gm,n,:r,k)
    add_constraint(gm, n, :compressor_ratio_value1, k, JuMP.@constraint(gm.model, r^2 * pi <= pj))
    add_constraint(gm, n, :compressor_ratio_value2, k, JuMP.@constraint(gm.model, r^2 * pi >= pj))
end
