...
function constraint_pipe_weymouth(
    ...
    _add_constraint!(
        ...
        :weymouth1,
        k,
        JuMP.@NLconstraint(gm.model, (pi - pj) <= (f * abs(f)) / w)
    )
    _add_constraint!(
        gm,
        n,
...
function constraint_pipe_weymouth_ne(
    ...
    _add_constraint!(
        ...
        :weymouth_ne1,
        k,
        JuMP.@NLconstraint(gm.model, w * (pi - pj) >= f * abs(f) + (1 - zp) * w * pd_min)
    )
    _add_constraint!(
        gm,
        n,
        :weymouth_ne2,
        k,
        JuMP.@NLconstraint(gm.model, w * (pi - pj) <= f * abs(f) + (1 - zp) * w * pd_max)
    )
end
    ...
    if type == 0
        if (min_ratio <= 1.0 && max_ratio >= 1)
            ...
            _add_constraint!(
                ...
                :compressor_ratios3,
                k,
                JuMP.@constraint(gm.model, f * (pi - pk) <= 0)
            )
            _add_constraint!(
                gm,
                n,
            ...
            _add_constraint!(
                ...
                :compressor_ratios6,
                k,
                JuMP.@constraint(gm.model, -f * (pj - pk) <= 0)
            )
    ...
    elseif type == 1
        ...
        if min_ratio == 1
            ...
            _add_constraint!(
                ...
                :compressor_ratios3,
                k,
                JuMP.@constraint(gm.model, f * (pi - pj) <= 0)
            )
    ...
    elseif type == 1
        ...
        if min_ratio == 1
            ...
            _add_constraint!(
                ...
                :compressor_ratios_ne3,
                k,
                JuMP.@constraint(gm.model, f * (pi - pj) <= (1 - zc))
            )
            # compression when flow is from i to j.  no compression when flow is from j to i. min_ratio != 1. This is a disjunctive model
        else
            y = get_ne_compressor_y_wp(gm, n, k)
