    ...
    if type == 0
        if (min_ratio <= 1.0 && max_ratio >= 1)
            ...
            _add_constraint!(
                ...
                :compressor_ratios1,
                k,
                JuMP.@constraint(gm.model, pk - max_ratio^2 * pi <= 0)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios2,
                k,
                JuMP.@constraint(gm.model, min_ratio^2 * pi - pk <= 0)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios3,
                k,
                JuMP.@constraint(gm.model, fpik <= 0)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios4,
                k,
                JuMP.@constraint(gm.model, pk - max_ratio^2 * pj <= 0)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios5,
                k,
                JuMP.@constraint(gm.model, min_ratio^2 * pj - pk <= 0)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios6,
                k,
                JuMP.@constraint(gm.model, -fpjk <= 0)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios7,
                k,
                JuMP.@constraint(gm.model, pjk == pj - pk)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios8,
                k,
                JuMP.@constraint(gm.model, pik == pi - pk)
            )
    ...
    elseif type == 1
        ...
        if min_ratio == 1
            ...
            _add_constraint!(
                ...
                :compressor_ratios3,
                k,
                JuMP.@constraint(gm.model, fpij <= 0)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios4,
                k,
                JuMP.@constraint(gm.model, pij == pi - pj)
            )
            ...
            _add_constraint!(
                ...
                :on_off_compressor_ratios2,
                k,
                JuMP.@constraint(gm.model, min_ratio^2 * pi - pj <= (1 - y) * (i_pmax^2))
            )
            _add_constraint!(
                gm,
                n,
...
function constraint_on_off_regulator_pressure(
    ...
    if max_ratio == 1
        ...
        _add_constraint!(
            ...
            k,
            JuMP.@constraint(gm.model, pj - max_ratio^2 * pi <= (1 - v) * j_pmax^2)
        )
        _add_constraint!(
            gm,
            n,
            ...
            JuMP.@constraint(
                gm.model,
                min_ratio^2 * pi - pj <= (1 - v) * (min_ratio * i_pmax^2)
            )
        )
        _add_constraint!(
            gm,
...
function constraint_compressor_ratios_ne(
    ...
    if type == 0
        if (min_ratio <= 1.0 && max_ratio >= 1)
            ...
            _add_constraint!(
                gm,
                n,
                :compressor_ratios_ne1,
                k,
                JuMP.@constraint(gm.model, pk - max_ratio^2 * pi <= (1 - zc) * k_pmax^2)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios_ne2,
                k,
                JuMP.@constraint(
                    gm.model,
                    min_ratio^2 * pi - pk <= (1 - zc) * i_pmax^2 * min_ratio^2
                )
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios_ne3,
                k,
                JuMP.@constraint(gm.model, fpik <= 0)
            ) # zc = 0 implies f = 0 so always true then
            _add_constraint!(
                gm,
                n,
                :compressor_ratios_ne4,
                k,
                JuMP.@constraint(gm.model, pk - max_ratio^2 * pj <= (1 - zc) * k_pmax^2)
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios_ne5,
                k,
                JuMP.@constraint(
                    gm.model,
                    min_ratio^2 * pj - pk <= (1 - zc) * j_pmax^2 * min_ratio^2
                )
            )
            _add_constraint!(
                gm,
                n,
                :compressor_ratios_ne6,
                k,
                JuMP.@constraint(gm.model, -fpjk <= 0)
            )  # zc = 0 implies f = 0 so always true then
            _add_constraint!(
            ...
            _add_constraint!(
                ...
                JuMP.@constraint(
                    gm.model,
                    pj - (max_ratio^2 * pi) <= (2 - y - zc) * j_pmax^2
                )
            )
            _add_constraint!(
                gm,
            ...
            _add_constraint!(
                ...
                JuMP.@constraint(
                    gm.model,
                    pi - (max_ratio^2 * pj) <= (1 + y - zc) * i_pmax^2
                )
            )
            _add_constraint!(
                gm,
    ...
    elseif type == 1
        _add_constraint!(
            ...
            :on_off_compressor_ratios_ne1,
            k,
            JuMP.@constraint(gm.model, pj - (max_ratio^2 * pi) <= (1 - zc) * j_pmax^2)
        )
        _add_constraint!(
            gm,
            n,
            ...
            JuMP.@constraint(
                gm.model,
                (min_ratio^2 * pi) - pj <= (1 - zc) * i_pmax^2 * min_ratio^2
            )
        )
        # compression when flow is from i to j.  no compression when flow is from j to i. min_ratio = 1
    else # type 2
        if min_ratio == 1
            ...
            _add_constraint!(
                ...
                JuMP.@constraint(
                    gm.model,
                    min_ratio^2 * pi - pj <= (1 - zc) * (min_ratio * i_pmax^2)
                )
            )
            ...
            _add_constraint!(
                ...
                :compressor_ratios_ne3,
                k,
                JuMP.@constraint(gm.model, fpij <= (1 - zc))
            )
            _add_constraint!(
                gm,
                n,
...
function constraint_compressor_ratio_value(
    ...
    if type == 0
        ...
        _add_constraint!(
            ...
            k,
            JuMP.@constraint(gm.model, rpi <= pj + (1 - y) * i_pmax^2 * max_ratio^2)
        )
        _add_constraint!(
            gm,
            n,
            ...
            k,
            JuMP.@constraint(gm.model, rpi >= pj - (1 - y) * j_pmax^2)
        )
        _add_constraint!(
            gm,
            n,
            ...
            k,
            JuMP.@constraint(gm.model, rpj <= pi + y * j_pmax^2 * max_ratio^2)
        )
        _add_constraint!(
            gm,
            n,
            ...
            k,
            JuMP.@constraint(gm.model, rpj >= pi - y * i_pmax^2)
        )
    else
...
function constraint_compressor_ratio_value_ne(
    ...
    if type == 0
        ...
        _add_constraint!(
            ...
            k,
            JuMP.@constraint(gm.model, rpi <= pj + (2 - y - z) * i_pmax^2 * max_ratio^2)
        )
        _add_constraint!(
            gm,
            n,
            ...
            k,
            JuMP.@constraint(gm.model, rpi >= pj - (2 - y - z) * j_pmax^2)
        )
        _add_constraint!(
            gm,
            n,
            ...
            k,
            JuMP.@constraint(gm.model, rpj <= pi + (1 + y - z) * j_pmax^2 * max_ratio^2)
        )
        _add_constraint!(
            gm,
            n,
            ...
            k,
            JuMP.@constraint(gm.model, rpj >= pi - (1 + y - z) * i_pmax^2)
        )
    else
        rpi = JuMP.@variable(gm.model)
        _IM.relaxation_product(gm.model, pi, r, rpi)
        _add_constraint!(
            ...
            :compressor_ratio_value_ne1,
            k,
            JuMP.@constraint(gm.model, rpi <= pj + (1 - z) * i_pmax^2 * max_ratio^2)
        )
        _add_constraint!(
            gm,
            n,
            :compressor_ratio_value_ne2,
            k,
            JuMP.@constraint(gm.model, rpi >= pj - (1 - z) * j_pmax^2)
        )
    end
end
