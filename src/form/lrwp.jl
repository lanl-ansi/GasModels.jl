...
"Constraint: Darcy-Weisbach equation - Linear Relaxation"
...
function constraint_pipe_weymouth_ne(gm::AbstractLRWPModel, n::Int, k, i, j, w, f_min, f_max, pd_min, pd_max)
    ...
    elseif (w == Inf) || is_flow_bounds_zero(f_min, f_max, gm.ref)
        ...
        # fmf_l incorporates the univariate relaxation for f*(abs(f))
        partition = get_flow_partition(gm, ne_pipe, "ne_pipe", f_min, f_max)
...
function constraint_compressor_ratio_value(gm::AbstractLRWPModel, n::Int, k, i, j, type, i_pmax, j_pmax, min_ratio, max_ratio)
    pi = var(gm, n, :psqr, i)
    pj = var(gm, n, :psqr, j)
    r = var(gm, n, :rsqr, k)
...
"Constraint: constrains the ratio to be ``p_i \\cdot \\alpha = p_j``"
function constraint_compressor_ratio_value_ne(gm::AbstractLRWPModel, n::Int, k, i, j, type, i_pmax, j_pmax, min_ratio, max_ratio)
    pi = var(gm, n, :psqr, i)
    pj = var(gm, n, :psqr, j)
