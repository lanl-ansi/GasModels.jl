...
function constraint_pipe_weymouth(gm::AbstractNLPModel, n::Int, k, i, j, f_min, f_max, w, pd_min, pd_max)
    ...
    pj = var(gm,n,:p,j)
    f  = var(gm,n,:f_pipe,k)
    add_constraint(gm, n, :weymouth1, k, JuMP.@NLconstraint(gm.model, (pi - pj) <= (f * abs(f))/w))
    add_constraint(gm, n, :weymouth2, k, JuMP.@NLconstraint(gm.model, (pi - pj) >= (f * abs(f))/w))
end
...
function constraint_pipe_weymouth_directed(gm::AbstractNLPModel, n::Int, k, i, j, w, f_min, f_max, directed)
    ...
    if directed == 1
        add_constraint(gm, n, :weymouth_ne1, k, JuMP.@constraint(gm.model, w*(pi - pj) >= f^2))
        add_constraint(gm, n, :weymouth_ne2, k, JuMP.@constraint(gm.model, w*(pi - pj) <= f^2))
    else
        add_constraint(gm, n, :weymouth_ne3, k, JuMP.@constraint(gm.model, w*(pj - pi) >= f^2))
        add_constraint(gm, n, :weymouth_ne4, k, JuMP.@constraint(gm.model, w*(pj - pi) <= f^2))
    end
end
...
function constraint_pipe_weymouth_ne_directed(gm::AbstractNLPModel,  n::Int, k, i, j, w, pd_min, pd_max, f_min, f_max, direction)
    ...
    if direction == 1
        add_constraint(gm, n, :weymouth_ne1, k, JuMP.@constraint(gm.model, w*(pi - pj) >= f^2 + (1-zp) * w * pd_min))
        add_constraint(gm, n, :weymouth_ne2, k, JuMP.@constraint(gm.model, w*(pi - pj) <= f^2 + (1-zp) * w * pd_max))
    else
        add_constraint(gm, n, :weymouth_ne3, k, JuMP.@constraint(gm.model, w*(pj - pi) >= f^2 - (1-zp) * w * pd_max))
        add_constraint(gm, n, :weymouth_ne4, k, JuMP.@constraint(gm.model, w*(pj - pi) <= f^2 - (1-zp) * w * pd_min))
    end
end
...
function constraint_pipe_weymouth_ne(gm::AbstractNLPModel,  n::Int, k, i, j, w, f_min, f_max, pd_min, pd_max)
    ...
    add_constraint(gm, n, :weymouth_ne1, k, JuMP.@NLconstraint(gm.model, w*(pi - pj) >= f * abs(f) + (1-zp) * w * pd_min))
    add_constraint(gm, n, :weymouth_ne2, k, JuMP.@NLconstraint(gm.model, w*(pi - pj) <= f * abs(f) + (1-zp) * w * pd_max))
end
...
function constraint_compressor_ratios(gm::AbstractNLPModel, n::Int, k, i, j, min_ratio, max_ratio, i_pmax, j_pmax)
    ...
    add_constraint(gm, n, :compressor_ratios3, k, JuMP.@constraint(gm.model, f*(pi - pj) <= 0))
end
...
function constraint_compressor_ratios_ne(gm::AbstractNLPModel, n::Int, k, i, j, min_ratio, max_ratio, f_max, i_pmin, i_pmax, j_pmin, j_pmax)
    ...
    #TODO this constraint is only valid if min_ratio = 1
    add_constraint(gm, n, :compressor_ratios_ne1, k, JuMP.@constraint(gm.model, pj - max_ratio^2*pi <= (1-zc)*j_pmax^2))
    add_constraint(gm, n, :compressor_ratios_ne2, k, JuMP.@constraint(gm.model, min_ratio^2*pi - pj <= (1-zc)*(min_ratio*i_pmax^2)))
    add_constraint(gm, n, :compressor_ratios_ne3, k, JuMP.@constraint(gm.model, f * (pi - pj) <= (1-zc) * f_max * M))
end
...
function constraint_on_off_control_valve_pressure(gm::AbstractNLPModel, n::Int, k, i, j, min_ratio, max_ratio, f_max, i_pmin, i_pmax, j_pmin, j_pmax)
    ...
    add_constraint(gm, n, :control_valve_pressure_drop2, k, JuMP.@constraint(gm.model, min_ratio^2*pi - pj <= (1-v)*(min_ratio*i_pmax^2)))
    add_constraint(gm, n, :control_valve_pressure_drop3, k, JuMP.@constraint(gm.model, f * (pi - pj) >= 0))
end
...
function constraint_compressor_ratio_value(gm::AbstractNLPModel, n::Int, k, i, j)
    ...
    pj    = var(gm,n,:p,j)
    r = var(gm,n,:r,k)
    add_constraint(gm, n, :compressor_ratio_value1, k, JuMP.@NLconstraint(gm.model, r * pi <= pj))
    add_constraint(gm, n, :compressor_ratio_value2, k, JuMP.@NLconstraint(gm.model, r * pi >= pj))
end
...
function constraint_compressor_energy(gm::AbstractNLPModel, n::Int, k, power_max, m, work)
    r = var(gm,n,:r,k)
    f = var(gm,n,:f_compressor, k)
    add_constraint(gm, n, :compressor_energy, k, JuMP.@NLconstraint(gm.model, f * (r^m - 1) <= power_max/work))
end
