...

{
    ...
    "title": "Gas Flow",
    "category": "section",
    "text": "GasModels implements a steady-state model of gas flow based on the Weymouth formulation that uses the 1-D hydrodynamic equations for natural gas flow in a pipe. Starting with the quasi-steady hydrodynamic equations and reducing them to a steady state model, the momentum conservation equation in a pipe is stated asp fracpartial ppartial x = -fraclambda a^2 phi phi2 Dwhere p is pressure (SI units are pascals), lambda is a non dimensional friction factor, phi is mass flux (SI units are kg/m^2/s), and D is the diameter of the pipe (SI units are m). Here, a^2=fracZRTm where Z is the gas compressibility factor (non-dimensional), R is the universal gas constant (SI units are J/mol/K), m is the molar mass of the gas (SI unites are kg/mol), and T is the gas temperature (SI units are K). In steady state, the mass conservation reduces to:    fracpartial phipartial x=0 where we have assumed the pipe area does not changing with x.  Integrating these equations from the start of the pipe at x=0 to the end of the pipe at x=L, where L is the length of the pipe, the equation for flux across the pipe is stated as    p^2(L)-p^2(0) = frac-lambda L a^2 phi phi 2 D  We typically express the mass flux through the pipe in terms of mass flow (SI units are m^3/s), f, where f=phi A. Here, A=fracpi D^24 is the cross-sectional area of the pipe (SI units are m^2). Thus, the equation for mass flow through the pipe is stated as     p^2(L)-p^2(0) = frac-lambda L a^2 f f 2 D A^2 Given potential numerical issues associated with these equations, it is very useful to non-dimensionalize the units. Here we use a  typical pressure p_0 and a typical mass flow f_0 and normalize the equations. This yields    tildep^2(L)-tildep^2(0) = -tildef tildef left(fraclambda L 2 Dright) left(fracf_0^2a^2A^2p_0^2right)where tildef=fracff_0 and tildep=fracpp_0 are the dimensionless mass flow and pressure, respectively, and are both of order one. Note that both terms in parenthesis on the right hand side of this equation are dimensionless.  For the purposes of convenience, we define resistance, w, as the constant w=left(fraclambda L 2 Dright) left(fracf_0^2a^2A^2p_0^2right)Finally, in most data sets, nodal injections and withdrawals are defined in terms of volumetric flow, q, at a STP conditions. Given this data, we non-dimensionalize based on q. At STP conditions, the mass flow is derived as f=fracqrho_s, where  rho_s is the gas density at STP conditions.More details of there derivations of these equations are found in Zlotnik, Chertkov, and Backhaus. _Optimal Control of Transient Flow in Natural Gas Networks_. CDC 2015. We note that this reference expresses these equations in terms of density, rho rather than pressure.  The transformation from density to pressure is simply rho=a^2p.A complete gas flow mathematical model is the defined bybeginaligned\ntextsets \n N  textjunctions \n A^p  textpipes  \n A^c  textcompressors  \n A^v  textvalves  \n A = A^p cup A^c cup A^v  textedges   \n P P_i  textproducers and producers at junction i   \n C C_i  textconsumers and consumers at junction i    \n\ntextdata \n w_a  textresistance factor of pipeline a \n fl_j  textconsumption (mass flux) at consumer j \n fg_j  textproduction (mass flux) at producer j \n underlinealpha_a=1 overlinealpha_a  text(de)compression limits (squared) of edge a \n underlinep_i ge 0overlinep_i  textlimits on pressure squared at node i \n\ntextvariables \n p_i  textpressure squared at node i \n f_a  textflux on edge a \n alpha_a  textcompression ratio on compressor a\n v_a  textvalve status for valve a 1 if valve is open\n\ntextconstraints \n (p_i - p_j) = w_a f_af_a textWeymouth equation for pipe a \n textconnected from junction i to junction j  \nsumlimits_a=a_ijin A f_a - sumlimits_a=a_ji in A f_a = sum_j in P_i fg_j- sum_j in C_i fl_j  textmass flux balance at junction i \n alpha_a p_i = p_j  textcompression boost at compressor a \ntextmass flux balance at junction i \n f_a (1-alpha_a) le 0 textcompression ratio is forced to 1 \n textwhen flow is reversed through compressor a \nunderlinep_i leq p_i leq overlinep_i  textpressure limits at junction i \nunderlinealpha_a leq alpha_a leq overlinealpha_a  textcompression limits at compressor i \n-v_a M leq f_a leq v_a M  textonoff operations for valve a \n textwhere M is the maximum flow through the valve \np_j - v_a overlinep_j leq p_i leq p_j + v_a overlinep_i  textlinks junction pressures of valve a \n textconnected from junction i to junction j\nendalignedmost of the optimization models of GasModels are variations of this formulation. In practice, we discretize on flow direction to reduce the non convexities of this model and relax the assumption that the minimum compression ratio is 1"
},
