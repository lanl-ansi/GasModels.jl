@testset "test nels" begin
    ...
    @testset "test misocp nels" begin
        @testset "gaslib 40 case" begin
            result = run_nels("../test/data/matgas/gaslib-40-nels.m", MISOCPGasModel, cvx_minlp_solver)
    ...
    @testset "test gaslib 40 misocp nels directed" begin
        ...
        @test isapprox(result["objective"] * result["solution"]["base_flow"] , 1252.95138889; atol = 1e-1)
    end
    ...
    @testset "test gaslib 40 mip nels directed" begin
        ...
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
        @test isapprox(result["objective"] * result["solution"]["base_flow"] , 1371.8466068710943; atol = 1e-1)
    end
    ...
    @testset "test gaslib 40 nlp nels directed" begin
        ...
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
        @test isapprox(result["objective"] * result["solution"]["base_flow"] , 1254.3181889121502; atol = 1e-1)
    end
end
