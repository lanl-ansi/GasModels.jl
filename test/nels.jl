...
@testset "test misocp nels directed" begin
    @testset "gaslib 40 case" begin
        ...
        @test isapprox(result["objective"] * result["solution"]["baseQ"] , 1252.95138889; atol = 1e-1) # conversion from 10^6 cubic meters per day to cubic meters per second
    end
end
...
@testset "test nlp nels directed" begin
    @testset "gaslib 40 case" begin
        ...
        result = run_nels_directed("../test/data/gaslib-40-nelsfd.json", NLPGasModel, cvx_minlp_solver)
        @test result["status"] == :LocalOptimal || result["status"] == :Optimal || result["status"] == :Suboptimal
        @test isapprox(result["objective"] * result["solution"]["baseQ"] , 1560.1204003868688; atol = 1e-1)
    end
end
