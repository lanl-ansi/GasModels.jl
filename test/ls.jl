@testset "test ls" begin
    ...
    @testset "test gaslib 40 misocp ls" begin
        ...
        result = solve_ls("../test/data/matgas/gaslib-40-ls.m", MISOCPGasModel, cvx_minlp_solver)
    ...
    @testset "test gaslib 40 misocp ls priority" begin
        ...
        result = solve_ls("../test/data/matgas/gaslib-40-ls-priority.m", MISOCPGasModel, cvx_minlp_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
        # After relaxation
    ...
    @testset "test gaslib 40 mip ls" begin
        ...
        result = solve_ls("../test/data/matgas/gaslib-40-ls.m", MIPGasModel, cvx_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
    ...
    @testset "test gaslib 40 mip ls priority" begin
        ...
        result = solve_ls("../test/data/matgas/gaslib-40-ls-priority.m", MIPGasModel, cvx_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
    ...
    @testset "test gaslib 40 lp ls" begin
        ...
        result = solve_ls("../test/data/matgas/gaslib-40-ls.m", LPGasModel, cvx_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
        @test isapprox(result["objective"]*result["solution"]["base_flow"], 817.75; atol = 1e-1)
    end
    ...
    @testset "test gaslib 40 lp ls priority" begin
        ...
        result = solve_ls("../test/data/matgas/gaslib-40-ls-priority.m", LPGasModel, cvx_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
        @test isapprox(result["objective"]*result["solution"]["base_flow"], 735.98; atol = 1e-1)
    end
    ...
    @testset "test gaslib 40 nlp ls" begin
        ...
        result = solve_ls("../test/data/matgas/gaslib-40-ls.m", NLPGasModel, cvx_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
    ...
    @testset "test gaslib 40 nlp priority" begin
        ...
        result = solve_ls("../test/data/matgas/gaslib-40-ls-priority.m", NLPGasModel, cvx_solver)
