@testset "test ls" begin
    ...
    @testset "test gaslib 40 mip ls" begin
        ...
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
        @test isapprox(result["objective"]*result["solution"]["base_flow"], 828.241566956375; atol = 1e0)
    end
    ...
    @testset "test gaslib 40 mip ls priority" begin
        ...
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
        @test isapprox(result["objective"]*result["solution"]["base_flow"], 745.3635379170676; atol = 1e-1)
    end
    ...
    @testset "test gaslib 40 lp ls priority" begin
        ...
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
        @test isapprox(result["objective"]*result["solution"]["base_flow"], 745.5144120833442; atol = 1e-1)
    end
    ...
    @testset "test gaslib 40 nlp ls" begin
        ...
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
        @test isapprox(result["objective"]*result["solution"]["base_flow"], 378.80800085143744; atol = 1e0)
    end
