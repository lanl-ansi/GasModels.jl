...
@testset "transient (steady state replicate) case with storage" begin
    ...
    @test isapprox(result["solution"]["nw"]["2"]["receipt"]["1"]["injection"], 43.02, atol = 1)
    @test isapprox(result["solution"]["nw"]["1"]["storage"]["1"]["storage_flow"], 86.45, atol = 1)
...
@testset "transient time-periodic withdrawal case with storage" begin
    ...
    @test isapprox(result["solution"]["nw"]["1"]["storage"]["1"]["storage_flow"], 86.51, atol = 1)
    @test isapprox(result["solution"]["nw"]["2"]["storage"]["1"]["storage_flow"], 87.96, atol = 1)
end
