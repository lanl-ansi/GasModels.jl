@testset "transient parsing" begin
    ...
    mn_data = parse_files(
        "../test/data/matgas/case-6.m",
        "../test/data/transient/time-series-case-6b.csv",
...
@testset "transient (steady state replicate) case" begin
    ...
    @test isapprox(
        result["solution"]["nw"]["2"]["receipt"]["1"]["injection"],
        94.80,
        atol = 1e-1,
    )
    @test isapprox(
...
@testset "transient (steady state replicate) case with storage" begin
    ...
    @test isapprox(
        result["solution"]["nw"]["1"]["storage"]["1"]["withdrawal"],
        -86,
        atol = 1,
    )
    @test isapprox(
        result["solution"]["nw"]["2"]["storage"]["1"]["withdrawal"],
        -86,
        atol = 1,
    )
end
