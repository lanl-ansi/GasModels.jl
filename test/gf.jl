...
@testset "test gf" begin
    @testset "test misocp gf" begin
        @testset "gaslib 40 misocp gf" begin
            ...
            result = solve_gf("../test/data/matgas/gaslib-40.m", MISOCPGasModel, cvx_minlp_solver)
        ...
        #    result = solve_gf("../test/data/gaslib-135.m", MISOCPGasModel, cvx_minlp_solver)
        #    @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
        #    @test isapprox(result["objective"], 0; atol = 1e-6)
    ...
    @testset "test mip gf" begin
        @testset "gaslib 40 mip gf" begin
            ...
            result = solve_gf("../test/data/matgas/gaslib-40.m", MIPGasModel, cvx_minlp_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
            @test isapprox(result["objective"], 0; atol = 1e-6)
        end
        ...
        @testset "gaslib 135 mip gf" begin
            ...
            result = solve_gf("../test/data/matgas/gaslib-135.m", MIPGasModel, cvx_minlp_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
            @test isapprox(result["objective"], 0; atol = 1e-6)
        end
    end
    ...
    @testset "test lp gf" begin
        @testset "gaslib 40 lp gf" begin
            ...
            result = solve_gf("../test/data/matgas/gaslib-40.m", LPGasModel, cvx_solver)
        ...
        @testset "gaslib 135 lp gf" begin
            ...
            result = solve_gf("../test/data/matgas/gaslib-135.m", LPGasModel, cvx_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
            @test isapprox(result["objective"], 0; atol = 1e-6)
        end
    end
    ...
    @testset "test nlp gf" begin
        @testset "gaslib 40 nlp gf" begin
            ...
            result = solve_gf("../test/data/matgas/gaslib-40.m", NLPGasModel, cvx_minlp_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
            @test isapprox(result["objective"], 0; atol = 1e-6)
        end
        ...
        @testset "gaslib 135 nlp gf" begin
            ...
            result = solve_gf("../test/data/matgas/gaslib-135.m", NLPGasModel, cvx_minlp_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == ALMOST_LOCALLY_SOLVED
            @test isapprox(result["objective"], 0; atol = 1e-6)
        end
    end
