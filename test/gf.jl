...
@testset "test minlp gf mathematical program" begin
    ...
    for (i,junction) in gm.ref[:junction]
        ...
        for constraint in set
            if i == 100
                @test string(constraint) == "-f[128] == 0.0555264"
            elseif i == 306
                @test string(constraint) == "-f[426] - f[77] + f[78] == 0"
            elseif i == 26
                @test string(constraint) == "-f[360] - f[269] == -45.4464"              
            end            
        end
        ...
        ## These checks could fail if we flip the orientation of the edge arbitrarly (i.e. :junction_connections has a different ordering)  Basically, there are equivlent 
        ## constraints if the edge flips.  We could put an || on those
