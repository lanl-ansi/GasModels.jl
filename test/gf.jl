...
@testset "test minlp gf mathematical program" begin
    ...
    @test isapprox(c.ub, 0.64266; atol = 1e-4)
    @test JuMP.sense(c) == :(==)
    ...
    c = gm.model.linconstr[ref.idx]
    @test isapprox(c.ub, -526; atol = 1e-4)
    @test JuMP.sense(c) == :(==)
    ...
    # 230.06619763449115 yp[186] - f[186] <= 230.06619763449115
    # f[186] + 230.06619763449115 yn[186] <= 230.06619763449115
    ref = gm.con[:nw][gm.cnw][:on_off_pipe_flow_direction1][186]
    c = gm.model.linconstr[ref.idx]
    @test isapprox(c.ub, 230.06619763449115; atol = 1e-4)
    @test JuMP.sense(c) == :<=
    @test length(c.terms.coeffs) == 2                
    for i = 1:length(c.terms.vars)
        if c.terms.vars[i] == gm.var[:nw][gm.cnw][:yp][186]
            @test isapprox(c.terms.coeffs[i], 230.06619763449115; atol = 1e-4)
    ...
    # 1882.584361111111 yp[321] - f[321] <= 325.31057760000004
    # f[321] + 1882.584361111111 yn[321] <= 325.31057760000004
   ref = gm.con[:nw][gm.cnw][:on_off_short_pipe_flow_direction1][321]
   c = gm.model.linconstr[ref.idx]
   @test isapprox(c.ub, 1882.584361111111; atol = 1e-4)
   @test JuMP.sense(c) == :<=
   @test length(c.terms.coeffs) == 2                
   for i = 1:length(c.terms.vars)
       if c.terms.vars[i] == gm.var[:nw][gm.cnw][:yp][321]
           @test isapprox(c.terms.coeffs[i], 1882.584361111111; atol = 1e-4)
    ...
    # 1882.584361111111 yp[549] - f[549] <= 1882.584361111111
    # f[549] + 1882.584361111111 yn[549] <= 1882.584361111111
    ...
    # -1882.584361111111 v[558] - f[558] <= 0
    # 1882.584361111111 yp[558] - f[558] <= 1882.584361111111
    # f[558] + 1882.584361111111 yn[558] <= 1882.584361111111
    ...
    # -1882.584361111111 v[591] - f[591] <= 0
    # f[591] + 325.31057760000004 yn[591] <= 1882.584361111111
    # 1882.584361111111 yp[591] - f[591] <= 1882.584361111111
    # f[591] - 1882.584361111111 v[591] <= 0
