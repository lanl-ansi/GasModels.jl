@testset "test ls" begin
    ...
    @testset "test gaslib 40 misocp ls" begin
        ...
        result = run_ls("../test/data/matgas/gaslib-40-E-ls.m", MISOCPGasModel, misocp_solver)
    ...
    @testset "test gaslib 40 misocp ls priority" begin
        ...
        result = run_ls("../test/data/matgas/gaslib-40-E-ls-priority.m", MISOCPGasModel, misocp_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
    ...
    @testset "test gaslib 40 mip ls priority" begin
        ...
        result = run_ls("../test/data/matgas/gaslib-40-E-ls-priority.m", MIPGasModel, mip_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
    ...
    @testset "test gaslib 40 lp ls" begin
        ...
        result = run_ls("../test/data/matgas/gaslib-40-E-ls.m", LPGasModel, lp_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
    ...
    @testset "test gaslib 40 lp ls priority" begin
        ...
        result = run_ls("../test/data/matgas/gaslib-40-E-ls-priority.m", LPGasModel, lp_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
        @test isapprox(result["objective"]*result["solution"]["base_flow"], 754.88; atol = 1e-1)
    end
    ...
    @testset "test gaslib 40 nlp ls" begin
        ...
        result = run_ls("../test/data/matgas/gaslib-40-E-ls.m", NLPGasModel, nlp_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL || result["termination_status"] == :Suboptimal
    ...
    @testset "test gaslib 40 nlp priority" begin
        ...
        result = run_ls("../test/data/matgas/gaslib-40-E-ls-priority.m", NLPGasModel, nlp_solver)
