...
@testset "test misocp gf" begin
    @testset "gaslib 582 case" begin
        ...
        result = run_gf("../examples/data/ne/gaslib-582.json", MISOCPGasModel, misocp_solver)
...
@testset "test minlp gf" begin
        @testset "gaslib 40 case" begin
            ...
            result = run_gf("../examples/data/matlab/gaslib-40.m", MINLPGasModel, minlp_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
            @test isapprox(result["objective"], 0; atol = 1e-6)
        end
...
@testset "test nlp gf" begin
        @testset "gaslib 40 case" begin
            ...
            result = run_gf("../examples/data/matlab/gaslib-40.m", NLPGasModel, minlp_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
            @test isapprox(result["objective"], 0; atol = 1e-6)
        end
#        @testset "gaslib 135 case" begin
#            println("gaslib 135 - NLP")
#            result = run_gf("../examples/data/matlab/gaslib-135.m", NLPGasModel, minlp_solver)
#            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
#            @test isapprox(result["objective"], 0; atol = 1e-6)
#        end
end
