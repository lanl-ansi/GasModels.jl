@testset "test misocp ne gaslib 582" begin
    ...
    @testset "gaslib 582 case 5%" begin
        ...
        result = run_ne("../examples/data/matgas/gaslib-582-G-5.m", MISOCPGasModel, misocp_solver)
    ...
    @testset "gaslib 582 case 10%" begin
        ...
        result = run_ne("../examples/data/matgas/gaslib-582-G-10.m", MISOCPGasModel, misocp_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
        @test isapprox(result["objective"], 0.0; atol = 1e-2)
        GC.gc()
    end
    ...
    @testset "gaslib 582 case 25%" begin
        ...
        result = run_ne("../examples/data/matgas/gaslib-582-G-25.m", MISOCPGasModel, misocp_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
        @test isapprox(result["objective"], 0.0; atol = 1e-2)
        GC.gc()
    end
    ...
    @testset "gaslib 582 case 50%" begin
        ...
        result = run_ne("../examples/data/matgas/gaslib-582-G-50.m", MISOCPGasModel, misocp_solver)
        @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == OPTIMAL
     ...
     @testset "gaslib 582 case 200%" begin
         ...
         result = run_ne("../examples/data/matgas/gaslib-582-G-200.m", MISOCPGasModel, misocp_solver)
     ...
     @testset "gaslib 582 case 300%" begin
         ...
         result = run_ne("../examples/data/matgas/gaslib-582-G-300.m", MISOCPGasModel, misocp_solver)
         @test result["termination_status"] == INFEASIBLE || result["termination_status"] == INFEASIBLE_OR_UNBOUNDED
         GC.gc()
     end
